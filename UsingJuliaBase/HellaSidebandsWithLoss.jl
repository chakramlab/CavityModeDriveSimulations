...
SC.Dynamics.RunPulseSequence(Mode3, ρ, fill("sb_f0g1", 1), c_ops = c_ops, run_name = "HellaSidebands"*string(with_loss)*"_"*string(now()), solver_kwargs = solver_kwargs, spns = 5)
end_time = now()
